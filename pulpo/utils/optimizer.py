...
class pyomo_base_model(pyo.AbstractModel):
    ...
    def _declare_sets(self):
        ...
        self.INTERVENTIONS = pyo.Set(doc='Set of (relevant) environmental interventions, indexed by k')
        self.INDICATOR = pyo.Set(doc='Set of impact assessment indicators, indexed by h') # FL: 2002 book has inconsistent nomenclature for this
        self.INTERVENTIONS_PROCESS = pyo.Set(within=self.INTERVENTIONS * self.PROCESS * self.INDICATOR, doc='Relation set between environmental cost flows and processes')
        self.INTERVENTIONS_IN = pyo.Set(self.INDICATOR, within=self.INTERVENTIONS)
        self.INTERVENTIONS_OUT = pyo.Set(self.INTERVENTIONS * self.INDICATOR, within=self.PROCESS)
        ...
        self.Intervention_in_out = pyo.BuildAction(rule=self._populate_intervention)
        self.Process_in_out = pyo.BuildAction(rule=self._populate_process)

    # Rule functions
    # ATTN: I have not worked with pyo.BuildAction, hence I dont know if these methods can be static methods.
    @staticmethod
    def _populate_intervention(model):
        """Relates the environmental flows to the processes."""
        for i, j, h in model.INTERVENTION_PROCESS:
            if i not in model.INTERVENTION_IN[h]:
                model.INTERVENTION_IN[h].add(i)
            model.INTERVENTION_OUT[i, h].add(j)
    @staticmethod
    def _populate_process(model):
    ...
    def _declare_parameters(self):
        ...
        self.INTERVENTION_MATRIX = pyo.Param(self.INTERVENTION_PROCESS, mutable=True, doc='Enviornmental cost matrix Q*B describing the environmental cost flows e associated to process j')
    ...
    def _impact_constraint(model, h):
        ...
        return model.impacts[h] == sum(sum(model.INTERVENTION_MATRIX[i, j, h] * model.scaling_vector[j] for j in model.INTERVENTION_OUT[i, h]) for i in model.INTERVENTION_IN[h])
    @staticmethod
...
class pyomo_intervention_limit_model(pyomo_base_model):
    """
    Description:
        the special case where the practitioner wants to specify limits on environmental interventions.
    """
    def __init__(self, *args, **kwds):
        super().__init__(*args, **kwds)

    def build_intervention_limit_model(self):
        self.build_base_model()
        self._change_to_intervention_model()

    def _change_to_intervention_model(self):
        self.INTERVENTION_LIMIT = pyo.Set(doc='Set of constrained environmental interventions, indexed by k')
        self.INTERVENTION_LIMIT_PROCESS = pyo.Set(within=self.INTERVENTION_LIMIT * self.PROCESS , doc='Relation set between constrained environmental interventions and processes')
        self.INTERVENTION_LIMIT_OUT = pyo.Set(self.INTERVENTION_LIMIT, within=self.PROCESS)

        self.INTERVENTION_LIMIT_MATRIX = pyo.Param(self.INTERVENTION_LIMIT_PROCESS, mutable=True, doc='Intervention matrix B describing the intervention flow k entering/leaving process j')
        self.UPPER_INV_LIMIT = pyo.Param(self.INTERVENTION_LIMIT, mutable=True, within=pyo.Reals, doc='Maximum intervention flow k')

        self.intervention_limit_vector = pyo.Var(self.INTERVENTION_LIMIT, bounds=(-1e24, 1e24), doc='Constrained intervention flows')

        self.INTERVENTION_LIMIT_CNSTR = pyo.Constraint(self.INTERVENTION_LIMIT, rule=self._intervention_limit_constraint)

        self.INTERVENTION_LIMIT_UPPER_CNSTR = pyo.Constraint(self.INTERVENTION_LIMIT, rule=self._intervention_limit_upper_constraint)

        def populate_inv(self):
            """Relates the impacts to the environmental flows"""
            for a, j in self.INTERVENTION_LIMIT_PROCESS:
                self.INTERVENTION_LIMIT_OUT[a].add(j)

        self.Intervention_limit_out = pyo.BuildAction(rule=populate_inv)

    @staticmethod
    def _intervention_limit_constraint(model, k):
        """Calculates the limited interventions flows"""
        return model.intervention_limit_vector[k] == sum(model.INTERVENTION_LIMIT_MATRIX[k, j] * model.scaling_vector[j] for j in model.INV_OUT[k])

    @staticmethod
    def _intervention_limit_upper_constraint(model, k):
        """Ensures that limited interventions are within capacities """
        return model.intervention_limit_vector[k] <= model.UPPER_INTERVENTION_LIMIT[k]
